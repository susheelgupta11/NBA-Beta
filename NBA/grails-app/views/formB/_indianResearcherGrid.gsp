	...
	<g:each in="${listOfIndianResearcher}" var="profile">
		<tr>
			...
			<% def listttt =  listOfIndianResearcher.size() - 1 %>
				<g:if test="${indianResearcher || deleteCalled }">
					<g:link controller="formB"
						action="editIndianResearcher" 
					...
					<g:if test="${listOfIndianResearcher?.size() == 1}">
						<g:link controller="applicationManagement" action="deleteIndianReasearcherProfile" onclick="return beforeDelete();"
					...
					<g:else>
						<g:link controller="formB" action="deleteIndianReasearcherProfile"
							params='[profileId:profile?.id,formType:formType,appFormId:appForm?.id,csrfPreventionSalt:"${csrfPreventionSalt}"]'>
							<img src="../../images/delete.png" width="16" height="16"
								alt="delete" title="delete" />
						</g:link>
				...
				<g:elseif test="${!indianResearcher }">
					<g:link controller="formB"
						action="editIndianResearcher" onSuccess="updateCountry(null,${profile?.nationality})" 
						params='[appFormId:appForm?.id,profileId:profile?.id,formType:formType,csrfPreventionSalt:"${csrfPreventionSalt}"]'>
						<img src="../images/edit.png" width="16" height="16" alt="edit"
							title="edit" />
					</g:link>
