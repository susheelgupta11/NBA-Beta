...
<body class="hold-transition skin-blue sidebar-mini">
 ...
 <modalbox:createLink action="create" params="[type:'priority']" title="Add Priority Value!" width="600">
   <input type="submit" value="Add" class="btn btn-primary pull-right" style="margin-bottom: 5px;"/>
</modalbox:createLink>
<span><b>Priority List</b></span>
...
<%--            <g:javascript library="application" /> --%>
           ...
           <g:each in="${setMstrDataInstance?}" var="setMstrData" status="i">
				<g:each in="${setMstrData?.priority}" var="priVal" status="j">
<tbody>
					<tr>
					<td>${j+1}</td>
					<td>${priVal}</td>
                          ...
                          <td>
                          <modalbox:modalIncludes />
								<modalbox:createLink action="edit" params="[type:'priority',index:j]" title="Update Priority Value!" width="600">
								<i class="fa fa-edit"style="font-size:24px;"></i>
								</modalbox:createLink>
                          
                          </td>
                          <td><g:link action="delete" params="[type:'priority',index:j,csrfPreventionSalt:csrfPreventionSalt]"><i class="fa fa-trash-o" style="font-size:24px;color:red"></i></g:link></td>
...
<%--	<script>--%>
<%--	var countForId = ${setMstrDataInstance?.priority.size()}--%>
<%--		function saveAja() {--%>
