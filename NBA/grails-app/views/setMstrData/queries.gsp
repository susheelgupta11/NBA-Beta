...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <modalbox:createLink action="create" params="[type:'quries']" title="Add Quries Value!" width="600">
   <input type="submit" value="Add" class="btn btn-primary pull-right" style="margin-bottom: 5px;"/>
</modalbox:createLink>
      <span><b>Query List</b></span>
     ...
     <g:each in="${setMstrDataInstance?}" var="setMstrData" status="i">
	       <g:each in="${setMstrData?.quries}" var="queVal" status="j">
				<tbody>
					<tr>
					<td>${j+1}</td>
					<td>${queVal}</td>
                          ...
                          <td>
                          <modalbox:modalIncludes />
								<modalbox:createLink action="edit" params="[type:'quries',index:j]" title="Update Quries Value!" width="600">
								<i class="fa fa-edit"style="font-size:24px;"></i>
								</modalbox:createLink>
                          
                          </td>
                          <td><g:link action="delete" params="[type:'quries',index:j,csrfPreventionSalt:csrfPreventionSalt]"><i class="fa fa-trash-o" style="font-size:24px;color:red"></i></g:link></td>
...
<%--	<script>--%>
<%--	var countForId = ${setMstrDataInstance?.quries.size()}--%>
<%--		function saveAja() {--%>
