...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <modalbox:createLink action="create" params="[type:'status']" title="Add Status Value!" width="600">
   <input type="submit" value="Add" class="btn btn-primary pull-right" style="margin-bottom: 5px;"/>
</modalbox:createLink>
<span><b>Status List</b></span>
        ...
        <table class="table table-bordered customtable">
           ...
           <g:each in="${setMstrDataInstance?}" var="setMstrData" status="i">
				<g:each in="${setMstrData?.status}" var="staVal" status="j">
<tbody>
					<tr>
					<td>${j+1}</td>
					<td>${staVal}</td>
                          ...
                          <td>
                          <modalbox:modalIncludes />
								<modalbox:createLink action="edit" params="[type:'status',index:j]" title="Update Status Value!" width="600">
								<i class="fa fa-edit" style="font-size: 24px;"></i>
								</modalbox:createLink>
                          
                          </td>
                          <td><g:link action="delete" params="[type:'status',index:j,csrfPreventionSalt:csrfPreventionSalt]"><i class="fa fa-trash-o" style="font-size: 24px; color: red;"></i></g:link></td>
...
<%--	<script>--%>
<%--	var countForId = ${setMstrDataInstance?.status.size()}--%>
<%--		function saveAja() {--%>
