...
<modalbox:createLink action="create" params="[type:'note']" title="Add Note Value!" width="600">
<input type="submit" value="Add" class="btn btn-primary pull-right" style="margin-bottom: 5px;" />
</modalbox:createLink>
  <span><b>Note List</b></span>
           ...
           <g:each in="${setMstrDataInstance?}" var="setMstrData" status="i">
				<g:each in="${setMstrData?.note}" var="noteVal" status="j">
<tbody>
					<tr>
					<td>${j+1}</td>
					<td>${noteVal}</td>
                          ...
                          <td>
                          <modalbox:modalIncludes />
								<modalbox:createLink action="edit" params="[type:'note',index:j]" title="Update Note Value!" width="600">
								<i class="fa fa-edit" style="font-size: 24px;"></i>
								</modalbox:createLink>
                          
                          </td>
                          <td><g:link action="delete" params="[type:'note',index:j,csrfPreventionSalt:csrfPreventionSalt]"><i class="fa fa-trash-o" style="font-size:24px;color:red"></i></g:link></td>
...
<%--	<script>--%>
<%--	var countForId = ${setMstrDataInstance?.note.size()}--%>
<%--		function saveAja() {--%>
