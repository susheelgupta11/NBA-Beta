...
<body class="hold-transition skin-blue sidebar-mini">
 ...
 <modalbox:modalIncludes />
  	<modalbox:createLink action="create" params="[type:'submission']" title="Add Submission Value!" width="600">
    <input type="submit" value="Add" class="btn btn-primary pull-right" style="margin-bottom: 5px;"/>
 </modalbox:createLink>
    
      <span><b>Submission List</b></span>
        <table class="table table-bordered customtable">
           ...
           <g:each in="${setMstrDataInstance?}" var="setMstrData" status="i">
				<g:each in="${setMstrData?.submission}" var="subVal" status="j">
              <tbody>
					<tr>
					<td>${j+1}</td>
					<td>${subVal}</td>
                          ...
                          <td>
                          <modalbox:modalIncludes />
								<modalbox:createLink action="edit" params="[type:'submission',index:j]" title="Update Submission Value!" width="600">
								<i class="fa fa-edit" style="font-size: 24px"></i>
								</modalbox:createLink>
                          </td>
                          
                          <td><g:link action="delete" params="[type:'submission',index:j,csrfPreventionSalt:csrfPreventionSalt]"><i class="fa fa-trash-o" style="font-size: 24px; color: red;"></i></g:link></td>
...
<%--	<script>--%>
<%--	var countForId = ${setMstrDataInstance?.submission.size()}--%>
<%--		function saveAja() {--%>
