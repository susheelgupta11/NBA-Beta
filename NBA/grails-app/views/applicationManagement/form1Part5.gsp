...
<body>
	...
	<g:javascript>
	var otpStatus = "${appForm?.otpStatus}"
							...
							<g:if test="${!declaration }">
								<div class="col-sm-6 form-group less-padding">
									<input type="radio" class="leftflot" name="radio"
										...
										class="leftflot"> &nbsp; <b>Declaration without EVC/OTP</b></span>
								</div>
								<div class="col-sm-6 form-group less-padding">
									<input type="radio" class="leftflot " name="radio" 
										value="digital" onclick="showhideDigitalSignature(this)" /><span
										class="leftflot"> &nbsp;<b>Declaration with EVC/OTP</b></span>
								</div>
							...
							<g:elseif test="${declaration}">
								<g:if test="${appForm?.isDeclarationDigitallySigned == 'true' }">
									<div class="col-sm-6 form-group less-padding">
										<input type="radio" class="leftflot" name="radio"
											value="withoutdigital"
											onclick="showhideDigitalSignature(this, otpStatus)" /><span
											class="leftflot"> &nbsp; <b>Declaration without EVC/OTP</b></span>
									</div>
								...
								<g:else>
									...
									<div class="col-sm-6 form-group less-padding">
										<input type="radio" class="leftflot " name="radio" 
											value="digital" onclick="showhideDigitalSignature(this, otpStatus)" /><span
...
<%--							 <g:form action= "uploadDigitalSignature">	--%>
<%--								<span class="col-sm-3">Upload signature:- </span>--%>
<%--								<div class="col-sm-4">--%>
<%--									<input type="file" name="digitalSignature" class="width" />--%>
<%--								</div>--%>
<%--								<div class="col-sm-4">--%>
...
<%--							 </g:form>	--%>
						...
						<div id="declarestatement" class="col-sm-12 form-group">
							<g:if test="${formType == 'Form1'}">
								<g:if test="${appForm?.submitCheck == true }">
									<h5>
										<g:checkBox class="checkbox" type="checkbox" checked="true"
											name="submitCheck" onclick="DisableSumitbtn(this)" style="float:left; min-width:26px;margin:-5px" />
								...
								<g:else>
									<h5>
										<g:checkBox class="checkbox" type="checkbox"
											name="submitCheck" onclick="DisableSumitbtn(this)" style="float:left; min-width:26px;margin:-5px" />
										<span style="width:97%"> I / We further declare that information provided in the
										application form is true and correct and I / we shall be
										responsible for any incorrect / wrong information.</span>
									</h5>
								</g:else>
							</g:if>
							<g:else>
								<g:if test="${appForm?.submitCheck == true }">
									<h5>
										<g:checkBox class="checkbox" type="checkbox" checked="true"
											name="submitCheck" onclick="DisableSumitbtn(this)" style="float:left; min-width:26px; margin:-5px" />
										<span style="width:97%"> I / We declare that information provided in the
										application form is true and correct and I / we shall be
										responsible for any incorrect / wrong information.</span>
									</h5>
								</g:if>
								<g:else>
									<h5>
										<g:checkBox class="checkbox" type="checkbox"
											name="submitCheck" onclick="DisableSumitbtn(this)" style="float:left; min-width:26px;margin:-5px" />
											<span style="width:97%">
										 I / We declare that information provided in the
										application form is true and correct and I / we shall be
										responsible for any incorrect / wrong information.
										</span>
									</h5>
							...
							<div class="col-sm-12 text-center">
								<g:hiddenField name="appFormId" value="${appForm?.id}" />
<%--								disabled="true"--%>
								<g:actionSubmit controller="applicationManagement" id="savebtn" 
									action="saveDeclaration" class="btn btn-primary" value="Save"/>
								<input type="button" value="Get OTP" onclick="otpSend(${appForm?.id});" style="display: none;" class="btn btn-primary" id="otpbtn">
								...
								<g:if test="${formType == 'Form1'}">
									<g:link controller="applicationManagement" id="preview"
										...
										disabled="true"
										params='[formType:formType,declaration:declaration,appFormId:appForm?.id,csrfPreventionSalt:"${csrfPreventionSalt}"]'>Preview</g:link>
								</g:if>
								...
								<input type="button" class="btn btn-primary sumitbtn" onclick="checkLeftTabsValidation()" id="submitFormFinally"
				...
				<div id="evcInstruction" style="display: none;">
				<ul>
                <li><b>Please make sure you have the correct Email ID 
					against your profile.<br>These details are important and 
					will be used for all future communications.</b></li>
				<li><b>You can update the details by going to
				    editing in the menu option “Profile Settings / My Profile’ .</b></li>
				</ul>
				</div>
				
				<div id="otpmsg" align="center"></div>
				
					<div align="center" style="margin-top: 5px; display: none;" id="chkotp">
	               In case you didn't received OTP <button type="button" onclick="otpSend(${appForm?.id});">Click here</button>
							<g:textField name="otptext" id="getOtp"/>
							<button type="button" class="btn btn-primary" name="${appForm?.id}" onclick="otpVerify(this);">Verify</button> 
							
					</div>
				<g:if test="${appForm?.otpStatus == false}">
				<script>
				$("#otpbtn").hide();
				</script>
				<div align="center" style="margin-top: 5px;" id="chkotp">
	               In case you didn't received OTP <button type="button" onclick="otpSend(${appForm?.id});">Click here</button>
							<g:textField name="otptext" id="getOtp"/>
							<button type="button" class="btn btn-primary" name="${appForm?.id}" onclick="otpVerify(this);">Verify</button> 
				...
				<g:elseif test="${appForm?.otpStatus == true}" >
				<script>
				$("#otpbtn").hide();
