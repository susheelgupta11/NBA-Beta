				...
				<div class="main-registeration col-sm-12">
					...
					<g:if test="${formType == 'Form1'}">
						<modalbox:createLink action="previewForm1Info"
							controller="FormPreview" title="Form I Preview" width="1000" params= '[csrfPreventionSalt:"${csrfPreventionSalt}"]'
							height="1200" id="${appForm?.id }">Click here to go to preview </modalbox:createLink>
						
					</g:if>
					<g:elseif test="${formType == 'Form2'}">
						<modalbox:createLink action="previewForm2Info"
							controller="FormPreview" title="Form II Preview" width="1000" params= '[csrfPreventionSalt:"${csrfPreventionSalt}"]'
							height="1200" id="${appForm?.id }">Click here to go to preview </modalbox:createLink>
					</g:elseif>
					<g:elseif test="${formType == 'Form3'}">
						<modalbox:createLink action="previewForm3Info"
							controller="FormPreview" title="Form III Preview" width="1000" params= '[csrfPreventionSalt:"${csrfPreventionSalt}"]'
							height="1200" id="${appForm?.id}">Click here to go to preview </modalbox:createLink>
							
					</g:elseif>
					<g:elseif test="${formType == 'Form4'}">
						<modalbox:createLink action="previewForm4Info"
							controller="FormPreview" title="Form IV Preview" width="1000" params= '[csrfPreventionSalt:"${csrfPreventionSalt}"]'
							height="1200" id="${appForm?.id}">Click here to go to preview </modalbox:createLink>
					</g:elseif>
					<div>
						<g:link controller="applicationManagement" action="form1Part5"
							params='[formType:formType,declaration:declaration,appFormId:params?.appFormId,csrfPreventionSalt:"${csrfPreventionSalt}"]'>Go back to submit.</g:link>
					</div>
