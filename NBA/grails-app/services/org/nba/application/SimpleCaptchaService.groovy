...
class SimpleCaptchaService {
	...
	boolean validateCaptcha(String captchaSolution) {
		...
		if (storeInSession()) {
			...
			println"kkkkkkkkkk"+solution
			println"LLLLLLLLLL"+captchaSolution
			...
			captchaSolution ? solution?.compareTo(captchaSolution) == 0 : false
			...
			if (response) {
             ...
             def rr  = encode(captchaSolution)
			captchaSolution ? solution == rr : false
		}
	}
	...
	String encode(String stringToEncode) {
		...
		new BigInteger(1, digest.digest(stringToEncode.bytes)).toString(16).padLeft(32, "0")
	}
